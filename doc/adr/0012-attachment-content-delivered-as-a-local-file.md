...
- `src/redmine/attachment_integration.test.ts` seeds an attachment with raw `POST /uploads.json` and `PUT /issues/:id.json` requests, because no port here creates one. It had not been run against a live Redmine when this record was written.
